...
final class KSyntaxParserRuby: KSyntaxParserProtocol {
    ...
    // MARK: Results
    private var _nodes: [_Node] = []                   // ascending by lowerBound

    // Pending (batched inserts before occasional merge)
    private var _pendingNodes: [_Node] = []
    ...
    nodeLoop:
        ...
        @inline(__always)
        func attrs(for kind: _Kind) -> [NSAttributedString.Key: Any]? {
            switch kind {
    ...
    func wordRange(at index: Int) -> Range<Int> {
        ...
        let chars = storage.characterSlice
        ...
        @inline(__always)
        func u(_ ch: Character) -> UInt32? { ch.unicodeScalars.first?.value }

        @inline(__always)
        func isAscii(_ ch: Character) -> Bool {
            ch.unicodeScalars.allSatisfy { $0.value <= 0x7F }
        }

        // Ruby 変数名の語頭: $, @, @@, または英字/_
        @inline(__always)
        func isRubyHead(_ ch: Character) -> Bool {
            guard let v = u(ch) else { return false }
            return v == 0x24 || v == 0x40 || // '$' or '@'
                   v == 0x5F ||              // '_'
                   (0x41...0x5A).contains(v) || (0x61...0x7A).contains(v)
        }

        // Ruby 識別子の継続: 語頭文字 or 数字
        @inline(__always)
        func isRubyCont(_ ch: Character) -> Bool {
            guard let v = u(ch) else { return false }
            return isRubyHead(ch) || (0x30...0x39).contains(v)
        }

        let pivot = chars[base + i]

        // $ や @ の単独も許す（カーソル位置によっては pivot が $/@ の場合）
        guard isRubyHead(pivot) || isRubyCont(pivot) else {
            return i..<i
        }

        // 変数名の開始を左に伸ばす
        var lo = i
        while lo > 0, isRubyCont(chars[base + lo - 1]) {
            lo -= 1
        }
        // $ や @ を見逃さないように、開始直前が $/@ ならそれも含める
        while lo > 0 {
            let prev = chars[base + lo - 1]
            if let v = u(prev), v == 0x24 || v == 0x40 { // '$' or '@'
                lo -= 1
            } else { break }
        }
        ...
        // 右方向に伸ばす
        var hi = i + 1
        while hi < n, isRubyCont(chars[base + hi]) { hi += 1 }

        // ? や ! が末尾にあれば追加
        if hi < n, let v = u(chars[base + hi]), v == 0x3F || v == 0x21 {
            hi += 1
        }
    ...
    private func _scan(range: Range<Int>, initialState: _ParserState) -> [_Node] {
        var nodes: [_Node] = []
        ...
        @inline(__always) func close(_ kind: _Kind, _ at: Int) {
            if spanStart >= 0 {
